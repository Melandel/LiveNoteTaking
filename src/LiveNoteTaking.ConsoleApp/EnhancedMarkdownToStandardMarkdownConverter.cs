...
partial class EnhancedMarkdownToStandardMarkdownConverter
{
	...
	private static partial Regex MindmapColoringFirstNodeRegex();
	[GeneratedRegex(@"^(\*\*|--|\+\+) ", RegexOptions.Multiline)]
	private static partial Regex MindmapColoringFirstRankChildrenRegex();
	[GeneratedRegex(@"^((?:\*|-|\+)+) ", RegexOptions.Multiline)]
	private static partial Regex MindmapColoringAllNodesRegex();
	[GeneratedRegex(@"^((?:\*|-|\+)+)(✘✘✘)", RegexOptions.Multiline)]
	private static partial Regex MindmapColoringTripleTodoNodesRegex();
	[GeneratedRegex(@"^((?:\*|-|\+)+)(✘✘)", RegexOptions.Multiline)]
	private static partial Regex MindmapColoringDoubleTodoNodesRegex();
	[GeneratedRegex(@"^((?:\*|-|\+)+)(✘)", RegexOptions.Multiline)]
	private static partial Regex MindmapColoringSingleTodoNodesRegex();
	[GeneratedRegex(@"^((?:\*|-|\+)+)(❓)", RegexOptions.Multiline)]
	private static partial Regex MindmapColoringQuestionMarkNodesRegex();
	[GeneratedRegex(@"\[([^\]]+)\]\(([^)]+)\)")]
	private static partial Regex MindmapHyperlinkImprovedSyntaxRegex();
...
title: {{
	...
	async Task<StandardMarkdownText> BuildSvgFromPlantumlSnippetWithTransparentBackground(EnhancedMarkdownText plantumlDiagramSnippet)
	{
		if (!_cachedGeneratedStandardMarkdown.ContainsKey(plantumlDiagramSnippet))
		{
			...
			diagramDescription.AppendLine(GetFinalEnhancedMarkdownText(plantumlDiagramSnippet, isMindMapDiagram));
	...
	EnhancedMarkdownText GetFinalEnhancedMarkdownText(EnhancedMarkdownText plantumlDiagramSnippet, bool isMindMapDiagram)
	{
		var diagramContent = string.Join(Environment.NewLine, plantumlDiagramSnippet.Split(Environment.NewLine)[1..^1]);
		if (isMindMapDiagram)
		{
			diagramContent = MindmapColoringFirstNodeRegex().Replace(diagramContent, @"$1[#8cb369] ");
			diagramContent = MindmapColoringFirstRankChildrenRegex().Replace(diagramContent, @"$1[#orange] ");
			diagramContent = MindmapColoringAllNodesRegex().Replace(diagramContent, @"$1[#lightblue] ");
			diagramContent = MindmapColoringTripleTodoNodesRegex().Replace(diagramContent, @"$1[#chocolate]$2");
			diagramContent = MindmapColoringDoubleTodoNodesRegex().Replace(diagramContent, @"$1[#seaGreen]$2");
			diagramContent = MindmapColoringSingleTodoNodesRegex().Replace(diagramContent, @"$1[#silver]$2");
			diagramContent = MindmapColoringQuestionMarkNodesRegex().Replace(diagramContent, @"$1[#tomato]$2");
			diagramContent = MindmapHyperlinkImprovedSyntaxRegex().Replace(diagramContent, "[[$2 $1]]");
		}
		return EnhancedMarkdownText.From(diagramContent);
	}
