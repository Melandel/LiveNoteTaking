...
class EnhancedMarkdownToStandardMarkdownConverter
{
	...
	public async Task<StandardMarkdownText> Convert(EnhancedMarkdownText markdownText)
	{
...
title: {{
	...
	async Task<StandardMarkdownText> BuildSvgFromPlantumlSnippetWithTransparentBackground(EnhancedMarkdownText plantumlDiagramSnippet)
	{
		if (!_cachedGeneratedStandardMarkdown.ContainsKey(plantumlDiagramSnippet))
		{
			...
			try
			{
				...
				if (firstLine.Length > EnhancedMarkdownText.DiagrammingEnhancement.Plantuml.StartToken.Length)
				{
					var title = firstLine[(EnhancedMarkdownText.DiagrammingEnhancement.Plantuml.StartToken.Length+1)..].Trim();
					if (!string.IsNullOrWhiteSpace(title))
					{
				...
				if (process.ExitCode == 1)
				{
					...
					{
						return BuildPlantumlHtmlFromPlantumlIncident(plantumlDiagramSnippet, error);
					}
				}
			}
			catch (Exception ex)
			{
				return BuildPlantumlHtmlFromPlantumlIncident(plantumlDiagramSnippet, ex.Message);
			}
			...
			if (diagramHasIncorrectSyntax)
			{
				return _cachedGeneratedStandardMarkdown.MostRecentlyCachedDiagramWithoutSyntaxError switch
				{
					null => EncapsulateDiagramWithParentDiv("plantuml", BuildPlantumlSyntaxErrorSnippet(plantumlErrorMessage)),
					...
					_ => EncapsulateDiagramWithParentDiv("plantuml", BuildPlantumlSyntaxErrorSnippet(plantumlErrorMessage))
				};
			}
	...
	StandardMarkdownText BuildPlantumlHtmlFromPlantumlIncident(string plantumlDiagramSnippet, string incidentMessage)
	{
		var html = new StringBuilder();
		html.AppendLine("<div class=\"generated-diagram-plantuml\">");
		html.AppendLine();
		html.AppendLine(InsertCommentIntoPlantumlSnippet(plantumlDiagramSnippet, $"☠️ {incidentMessage}"));
		html.AppendLine();
		html.AppendLine("</div>");
		html.AppendLine();
	...
	{
		var minifiedDiagramSvg = new HtmlMinifier().Minify(plantumlStdOutput.ToString()).MinifiedContent;
		return StandardMarkdownText.From(minifiedDiagramSvg);
	}

	StandardMarkdownText BuildPlantumlSyntaxErrorSnippet(string plantumlErrorMessage)
	{
		var errorAsMarkdownBlockSnippet = new StringBuilder(Environment.NewLine);
		errorAsMarkdownBlockSnippet.AppendLine("```");
